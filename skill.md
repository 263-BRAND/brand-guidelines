...
1. **色值** — 多系列数据只能用品牌色板内的颜色，禁止自造衍生色值（包括调亮/调暗/调饱和度）。色板颜色不够时靠线型/图案区分，不得自行生成。集团红可用：`#D0121B`(primary)、`#AC000A`(primaryDark)、`#FE343F`(primaryLight)、`#FF777F`(accent)
2. **安全区** — 图表整体（坐标轴、标签、图例）不得超出页面安全区（`top:12% ~ bottom:18%`），不得侵犯右上 Logo 安全区
3. **字号底线** — 图表标签 ≥ 20pt，坐标标注 ≥ 18pt
4. **禁止纯黑** — 图表文字使用 `#2D3847`（标题）/ `#595959`（标注）

### CSS 柱状图（已验证可靠）

- `<div>` + `linear-gradient` 构建柱体，圆角 4-6px，宽度一致。渐变色必须来自品牌色板（如 `#D0121B` → `#FE343F`）
- 柱顶标注数值（24pt，品牌红），底部标注类别（20pt，#595959）
- 网格线/轴线可用中性灰（`#EEEEEE`/`#E0E0E0`），不属于品牌色违规

### 数据表格（已验证可靠）

- 表头 `#F2F2F2` 底（lightGray）+ 2px `#D0121B` 分割线
- 斑马纹偶数行可用中性灰（`#FAFAFA`），数值列右对齐
- 峰值行高亮用品牌主色极低透明度（如 `rgba(208,18,27,0.06)`）+ 数值加粗

### SVG 折线图（已验证可靠）

- 所有元素放入单个 SVG，`viewBox="0 0 740 520"` 统一坐标系，禁止 CSS/SVG 混用
- `<polyline>` 折线 + `<circle>`/`<rect>` 数据点，终值实心圆高亮（r≥10）
- **字号**（SVG viewBox 内）：
  - 常规（≤8 个数据点）：轴标签 18、数据标签 16
  - 密集（>8 个数据点）：轴标签 16、数据标签 14
  - **绝对底线 12**（渲染后约 19pt，不低于 skill 18pt 下限）
- 验证通过：6 月单系列 / 3 系列 6 月 / 12 月单系列
