...
传统 PPT 模板是人读、人用的。在 agent 时代，企业销售工具文档需要让员工的 agent 能调用和执行，不同部门、不同业务、不同场景都能产出视觉风格统一的 PPT。

## 核心思路

**"只约束 VI，不限制布局。"**

- 我们交付的是一套 agent 可调的 VI 设计系统，不是 30 套固定页面模板
- Agent 负责内容策划和页面类型组合，VI 系统保证视觉一致性
...
**选择逻辑：** 页面底色为深色 → 反白稿，否则 → 彩稿。由渲染器自动执行。
