...
5. **封面风格（仅内部汇报时追加，此限定不对用户说）：** 用户话术：「封面风格：选**个性化风格**（Geek 像素动效）还是**严谨商务风格**？」输出格式为 PPT 时补充推荐：「如生成 PPTX，推荐**严谨商务风格**」。禁止向用户提及内部字段名（red-template）或「内部汇报专属」等内部限定。技术映射见下方「规则」→「确认结果必须写入 pages.json」。
