...
**以下 4 个问题必须逐个弹出确认，第 5 题（封面风格）仅在「内部汇报」时追加。如果你发现自己正在生成内容但还没问完这些必须确认的问题，立即停下来，回到确认步骤。**

1. **展示场景：** 内部汇报还是对外展示？
2. **配色方案：** 集团红还是商务蓝？（默认集团红。集团红对应集团 Logo，商务蓝对应云通信 Logo）
3. **视觉风格：** 简洁标准版式，还是调用专业设计能力美化排版？
4. **输出格式：** 网页文件（HTML）还是 PPT 文件（PPTX）？
5. **封面风格（仅内部汇报时追加）：** 个性化还是严谨？—— 个性化 → 字符画封面；严谨 → 红色正式封面。**提问用户时禁止提及内部字段名（如 red-template）**，仅用「个性化」「严谨」。当输出格式 = PPTX 时，**推荐严谨**（字符画封面和动态效果无法在 PPTX 原生渲染，严谨封面可 HTML/PPTX 100% 视觉一致），但最终由用户选择。技术映射见下方「规则」→「确认结果必须写入 pages.json」。
...
2. **硬门禁确认** → 向用户逐个弹出确认问题（见上方"生成前确认"章节，内部汇报时含封面风格第 5 题）。全部确认后才能继续。禁止跳过，禁止假设默认值
...
- 禁止向用户暴露 Template/Themed 等内部术语**及内部文件名**（如 red-template、cover-red-template.png、template-cover-bg.png、scene/background 等字段值）。**模式/封面术语**在用户对话中永远使用「内部汇报」「对外展示」「个性化」「严谨」（集团 Logo、HTML/PPTX 等正常用词不受此限）
