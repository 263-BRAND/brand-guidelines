...
### 用户对话措辞规范（对用户说任何话之前，先过这三条）

1. **逐字话术**：确认问题必须**逐字原样**使用下方「用户话术」，禁止自行发挥、禁止改写、禁止加解释、禁止翻译成其他说法。
2. **禁止播报判断依据**：不要向用户解释你是如何判断场景的——禁止出现「命中」「锚词」「锁定」「检测到」等推理词，也不要复述用户资料的原文来论证判断。判断过程只留在内部，对用户只输出结果话术。
3. **对话黑名单（出现即违规）**：

| 内部词汇/文件名（禁止对用户说） | 对用户只说 |
|---|---|
| Template / Themed | 内部汇报 / 对外展示 |
| 母版、硬规则、硬门禁、锚词、命中、锁定 | 不出现（不解释） |
| red-template、cover-red-template.png、template-cover-bg.png | 严谨商务风格 |
| scene、background、colorScheme、logoSet、pages.json | 不出现（内部字段名） |
| brand-tokens.json、company-data.json、generate.js、renderer | 不出现（内部文件名） |
| ASCII 字符画、二进制雨 | 个性化风格（Geek 像素动效） |

> 允许对用户说的正常词：集团 Logo、网页文件、PPT 文件、集团红、商务蓝、PPTX、正文、标题、副标题、页面、图表等业务/格式词汇。

**对用户说话时只允许输出以下用户话术；技术映射（字段值、文件名、pages.json 结构）只留在内部，绝不向用户展示。**

1. **展示场景：** 先判断场景（内部汇报/对外展示），再确认式提问（判断是 A 则询问是否需要改成 B）。判断为「内部汇报」→ 用户话术：「根据您提供的资料，我判断这个 PPT 用于**内部汇报**。如果实际是用于**对外展示**，请告诉我，我会切换设计风格。」判断为「对外展示」→ 话术反之。
2. **配色方案：** 用户话术：「用**集团红**还是**商务蓝**？」（默认集团红）
3. **视觉风格（仅对外展示时询问；内部汇报不询问，默认严格按模板）：** 用户话术：「**严格按模板排版**，还是调用**专业设计能力美化排版**？」选「严格按模板」→ 按母版锁定排版（设计 skill 不介入）；选「专业设计美化」→ 硬规则锁死、软规则由设计 skill 自由发挥。
4. **输出格式：** 用户话术：「要**网页文件**还是**PPT 文件**？」
5. **封面风格（仅内部汇报时追加，此限定不对用户说）：** 用户话术：「封面风格：选**个性化风格**（Geek 像素动效）还是**严谨商务风格**？」**推荐逻辑由第 4 题输出格式决定，双向强制：**
   - **输出格式为「PPT 文件」→ 必须补充推荐**：「如生成 PPT 文件，推荐**严谨商务风格**」（原因：个性化风格的像素动效在 PPT 中无法原生渲染，严谨风格是 PPT 的适配封面）
   - **输出格式为「网页文件」→ 禁止推荐严谨风格**——网页两个风格都原生支持，让用户自行选择，不得附加推荐语
   - 禁止向用户提及内部字段名（red-template）或「内部汇报专属」等内部限定。技术映射见下方「规则」→「确认结果必须写入 pages.json」。
...
- 禁止向用户暴露 Template/Themed 等内部术语**及内部文件名**（如 red-template、cover-red-template.png、template-cover-bg.png、scene/background 等字段值）。**模式/封面术语**在用户对话中永远使用「内部汇报」「对外展示」「个性化风格」「严谨商务风格」（集团 Logo、HTML/PPTX 等正常用词不受此限）。完整替换对照见「生成前确认 → 用户对话措辞规范 → 对话黑名单」
