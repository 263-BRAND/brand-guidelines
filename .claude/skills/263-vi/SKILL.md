...
4. **输出格式**：逐字说：「要**网页文件**还是**PPT 文件**？」选「PPT 文件」→ 先按工具能力分叉：有原生 PPTX 排版能力（专业PPT制作技能或设计类技能 / 工具原生 PPT）直接按该路径生成；只能代码生成才走 4a/4b 确定最终路径。选「网页文件」→ 直接第 5 题。第 5 题仅「工作汇报」分支询问（对外展示分支第 4 题即结束）。
4a. **PPT 输出路径·推荐**（仅第 4 题选「PPT 文件」且当前工具只能代码生成 PPT 时）：逐字说：「当前工具制作 PPT 文件需要**用Python代码生成**，效果可能不够稳定。推荐改生成**网页文件**，或者使用**专业PPT制作技能或设计类技能**。你希望怎么处理？」用户改选网页文件 → 最终格式 = 网页文件（第 5 题不补严谨推荐）；用户选专业能力 → 按该能力路径生成；用户坚持代码生成 → 走 4b。
...
1. **具备原生 PPTX 排版能力**（专业PPT制作技能或设计类技能 / 工具原生 PPT 生成）→ 优先用它，把品牌数据（尺寸/字号/行距/图片规则）作为契约交给它。
2. **无 → 推荐生成网页文件（HTML）**：交互见 4a，用户接受则按网页文件生成（可靠）。
