...
4. **生成**——按确认结果与页面规范产出（HTML 走 `generate.js`；PPTX 按「PPT 输出路径」分层决策）
...
1. **逐字话术**——上方编号话术必须逐字原样说出，禁止自行发挥、改写、加解释、翻译。
...
| brand-tokens.json、company-data.json、generate.js、renderer | 无——内部文件名 |
...
> 允许对用户说的正常词：集团 Logo、网页文件、PPT 文件、集团红、商务蓝、PPTX、正文、标题、副标题、页面、图表等业务/格式词汇。
...
**Node.js 可用性分支（渲染不因环境降级）：** 执行前先确认环境有 `node`。若环境无 node，**禁止跳过品牌约束或改用无约束的随意渲染**——回退按序：① 按「HTML 输出规范」章节手动生成**自包含 HTML**（全部品牌约束手动应用：色板从 token 读值、Logo 安全区、字号/行距、图片等比缩放，一条不少）；② 或输出 pages.json + 渲染说明，交用户/具备 node 的环境渲染。**任何情况下不得因渲染器不可用而放宽颜色、安全区、字号约束。**
