...
> 允许对用户说的正常词：集团 Logo、网页文件、PPT 文件、集团红、商务蓝、PPTX、正文、标题、副标题、页面、图表、广告法合规审查、违禁词、极限词等业务/格式/合规词汇。
...
**红底场景次级元素 = 白色（强制）：** 红底/红卡片（品牌红 `#D0121B` 或色阶深档填充）上的文字、次级强调一律用白色 `#FFFFFF`，**禁止用 accent 浅粉 `#FF777F`、色阶浅档（s1/s2/s3）或任何色板外浅色**——同属红系对比度极低，红底+浅粉是"死色"（视觉不可见）。层次感用白色分级（如白 100% / 白 80% 透明度）实现，**不靠引入色板外颜色**。
