...
version: 1.1.1
description: 263 品牌 VI 规范 v1.1.1 — 提供品牌色板/字体/Logo/slogan/公司数据与品牌生成流程。**生成前必须先逐字完成「生成前确认」编号话术（使用场景/配色/输出格式/视觉风格，按场景分支问全），未确认不得开始生成。** 只要用户要为 263/二六三 做工作汇报（汇报/述职/周报/月报/季报/年报）、对外展示（介绍/宣传/发布会/客户材料）或任何品牌 PPT/网页/文档，即使没提"品牌/VI"也应使用此 skill。
---
...
当用户需要输出涉及 263 品牌的内容时，你负责品牌数据层的全部决策。**铁律：任何生成动作之前，必须先完成「生成前确认」——按场景分支逐字问完编号话术（工作汇报 → 问 1/2/3；对外展示 → 问 1/2/3/4），全部确认后才能开始生成。判断只是内部动作，确认是硬性必问，禁止跳过、禁止假设默认值、禁止先动手再问。**
...
示例（仅供你内部理解，不对用户说）：用户说"帮我做个Q3工作汇报"→ 含「汇报」→ 判定为工作汇报 → 去「生成前确认」问 1/2/3。
...
确认问题是硬性必问——Agent 先判断场景，但判断后也必须把对应分支的问题逐一询问用户确认，禁止因已判断而跳过询问或直接生成。确认问题按场景分支：工作汇报 → 问 1/2/3；对外展示 → 问 1/2/3/4。如果你发现自己正在生成内容但还没问完这些必须确认的问题，立即停下来，回到确认步骤。
...
0. **版本声明**（每次生成前的**第一句话**，先于第 1 问逐字说出。`<版本号>` 填入你**实际加载**的 SKILL.md frontmatter `version` 字段值——版本每次发版会变化，禁止硬编码、禁止背诵旧值，加载的 SKILL.md 是几版就报几版，这是用户核对是否装对版本的依据）：「当前加载的品牌指南版本为 **<版本号>**。」
   若用户表示他预期的是其他版本（版本不符）或询问版本确认方法，逐字补充：「版本不符说明加载的是旧版 Skill。请**先删除已安装的旧版 Skill 目录**，再重新安装最新版，并**新开会话**后重试。」
...
3. **输出格式**：逐字说：「要**网页文件**还是**PPT 文件**？」选「PPT 文件」→ 检查本次对话中**已客观存在且可直接调用**的、能直接产出 .pptx 版式的**外部能力载体**——已加载的专业PPT制作技能或设计类技能、已定义的 agent、已安装插件的技能、已连接且授权可用的设计类 MCP 工具（客观可查的事实，不是你的能力自评）。**263group-brand-guidelines 本身不算**——它只提供品牌数据，产不出 .pptx 版式；通用代码执行类工具/MCP 也不算（仍是你的代码能力）：
   ...
   选「网页文件」→ 工作汇报直接开始生成；对外展示 → 第 4 题询问视觉风格。
...
| red-template、cover-red-template.png | 严谨商务风格 |
...
工作汇报封面（唯一）：红色设计位图底图 + 文字叠加，HTML/PPTX 100% 视觉一致。

**触发**：工作汇报（`scene:"template"`）即默认（唯一）封面，cover slide 无需 `background` 键（`scene:"template"` + cover 即可）。
...
**⚠️ 位图无烘焙 Logo（2026-08-24 修正）**：`cover-red-template.png` 是**纯设计位图（白底 + 右上红系几何），不包含任何集团 Logo**——封面 Logo 必须由渲染器/设计 skill 显式叠加。**禁止**以「位图已带 Logo」「避免 Logo 重叠」为由省略或删除封面 Logo；位图顶部为纯白，左上角叠加 Logo 与位图内容永不重叠。委托任何工具生成封面时，**封面 Logo 是独立必加元素，不是背景位图的一部分**。
...
2. **Logo**：封面必须带 263 品牌标识（PNG Logo）；Logo 安全区零容忍（左上角，`layout.coverLogo` 读取位置/尺寸）；**封面 Logo 一律彩稿（`logo-group-color`），禁止反白**——无论封面深浅底。深色封面彩稿不可读时调整布局（Logo 置于浅色区域），而非换反白
...
- **Template（对内汇报）**：`pages.scene === 'template'` → 红色位图封面（`red-template`）+ 左对齐文字（唯一封面）
...
- 封面背景选项（Template）：red-template（唯一）
...
1. 读取已有文件，提取页面结构和内容
2. **替换所有颜色为品牌色板色值**（不只是主色，包括所有辅助色：绿→品牌红（正向）或保留语义（负向/失败，同「改写功能 → 换色映射」），蓝→深色/辅色，灰→品牌灰）。Template 路径下原文件的一切非品牌颜色必须全部清除
...
| R3 | 双保险行为问法（**仅 HTML** 结构模糊时；PPTX 默认演示文稿不问；R2 与 agent 判断不一致走双保险「停下来描述让用户拍板」，不走 R3） | 「您希望阅读时是像 PPT 一样翻页的，还是像网页一样滚动阅读的？」 |
...
- 左右留白：≥ 5%
- 封面标题位置：HTML `top:48%`，水平居中

### PPTX 输出路径（第 3 题选「PPT 文件」后的决策）

PPTX 布局比 HTML 脆，不鼓励手写代码排版。按优先级决策（交互见「生成前确认 → 用户话术」3a/3b）：

1. **对话中已客观存在且可直接调用、能直接产出 .pptx 版式的外部能力载体**（已加载的专业PPT制作技能/设计类技能、已定义的 agent、已安装插件的技能、已连接且授权可用的设计类 MCP 工具） → 这就是原生 PPTX 排版能力，优先用它，把品牌数据（尺寸/字号/行距/图片规则）作为契约交给它。**263group-brand-guidelines 不算**（品牌数据层，只产品牌数据与 HTML）；你的代码能力、通用代码执行类 MCP 也不构成原生能力；agent 不能中途加载新能力——判断只看用户是否已加载/连接该能力，不是你的能力自评。**产出后校验见「PPTX 生成后自查」——此路径产出方式在设计技能手中，无法机器强制，靠清单人工核对（见 pptx-python-guide.md §9.2 强制边界）**。
...
4. **复杂溢出判据（改写时字号放大，按载体分流）**：PPTX——改动只落 1 个文本框且不重叠 → agent 调整框尺寸/位置即可；重叠或跨元素联动（牵动图表/图片位置）→ 转设计 skill 或退回用户。HTML——不用「跨元素联动」信号（文档流自动 reflow 是免费行为），只保留「不重叠」判断用于绝对定位/定高容器（封面标题/Logo 安全区/定高卡片），正常流元素 agent 直接处理；HTML 走 generate.js 时字号渲染器强制，此判据基本不触发。
...
| 封面 Logo（Template）：左上角集团彩稿 PNG Logo（`layout.coverLogo` 位置/尺寸，见「工作汇报红色封面」章节） | MUST |
...
**设计能力判定（与 PPTX 能力同一把客观尺子，HTML/PPTX 通用）：** 本次对话中**已客观存在且可直接调用**、能直接产出 HTML 或 PPTX 版式的**外部能力载体**——已加载的设计类技能（frontend-design、专业PPT制作技能或设计类技能）、已定义的 agent、已安装插件的技能、已连接且授权可用的设计类 MCP 工具。**263group-brand-guidelines 不算**——只产品牌数据与 pages.json，产不出表现层（HTML 与 PPTX 都如此）；**你的代码能力不算、通用代码执行类 MCP 不算**——任何工具都能写代码，手写 HTML/PPTX 属回退路径；**agent 不能中途加载新插件/MCP/agent**（安装/连接是用户侧配置动作）。仅加载 263group-brand-guidelines → 判定**无设计能力** → Themed 封面走渲染器兜底封面图（默认）。
...
## 模板注入契约（委托任何工具时随附，2026-08-24）

当 263group-brand-guidelines 委托**任何**外部工具（设计 skill、专业PPT制作技能、已定义 agent、插件技能、已授权设计类 MCP）生成表现层时，把本契约作为**自包含模板清单**随附给受托工具——受托工具不得凭直觉猜测下列任何一项，也不得因"看起来重复/位图已带"而省略。本契约是对上文各详细章节的委托摘要；详细规则以对应章节为准。

| 项 | 值（从 token 实时读取） | 刚性 |
|----|----|:--:|
| 画布 | HTML 1920×1080；PPTX 960×540pt；委托 SVG 中间态 1280×720（= 960pt 的 1.333× 源，**pt = px ÷ 0.75**） | MUST |
| 封面背景 | `cover-red-template.png` 全幅位图（工作汇报）；**位图无烘焙 Logo——封面 Logo 必须显式叠加，禁止省略/删除** | MUST |
| 封面 Logo | 彩稿 `logo-group-color`，左上角 left:6%/top:8%，正方形 125px@1920（PPTX 62pt @ 57.6/43.2；委托中间态 77/58/83@1280×720）；Logo 安全区零容忍 | MUST |
| 内页 Logo | 彩稿 `logo-group-color`，**右上角** right:5.2%/top:5.3%，80px@1920（PPTX 40pt；委托中间态 1160/38/53@1280×720） | MUST |
| 结尾页 | 居中彩稿 Logo（高 30%~36% 画布）+ 居中 slogan PNG（宽 45%~55% 画布，显式高度 = 宽度 ÷ 9.44），**必须最后一页** | MUST |
| 色板/字体/公司数据 | 从 `brand-tokens.json` / `company-data.json` 读取，禁手写 hex、禁凭记忆写数值 | MUST |
| 校验 | HTML → `brand-check-html.py`；PPTX → `brand-check-pptx.py`；设计工具产出路径 → 按「PPTX 生成后自查」清单人工核对 | MUST |

**委托时的硬约束**：品牌数据层（本 skill）定色板/字体/Logo/结尾页格式；表现层（受托工具）排版。受托工具产出的页面必须逐条对上表——封面有 Logo、内页 Logo 在右上、结尾页最后，缺一不可。
...
- 禁止向用户暴露 Template/Themed 等内部术语**及内部文件名**（如 red-template、cover-red-template.png、scene/background 等字段值）。**模式/封面术语**在用户对话中永远使用「工作汇报」「对外展示」「严谨商务风格」（集团 Logo、HTML/PPTX 等正常用词不受此限）。完整替换对照见「生成前确认 → 内部思考词汇」
