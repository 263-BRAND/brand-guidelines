...
description: 263 品牌 VI 规范 — 提供品牌色板/字体/Logo/slogan/公司数据与品牌生成流程。**生成前必须先逐字完成「生成前确认」编号话术（使用场景/配色/输出格式/视觉风格/封面风格，按场景分支问全），未确认不得开始生成。** 只要用户要为 263/二六三 做工作汇报（汇报/述职/周报/月报/季报/年报）、对外展示（介绍/宣传/发布会/客户材料）或任何品牌 PPT/网页/文档，即使没提"品牌/VI"也应使用此 skill。
---
...
当用户需要输出涉及 263 品牌的内容时，你负责品牌数据层的全部决策。**铁律：任何生成动作之前，必须先完成「生成前确认」——按场景分支逐字问完编号话术（工作汇报 → 问 1/2/3/5；对外展示 → 问 1/2/3/4），全部确认后才能开始生成。判断只是内部动作，确认是硬性必问，禁止跳过、禁止假设默认值、禁止先动手再问。**
...
示例（仅供你内部理解，不对用户说）：用户说"帮我做个Q3工作汇报"→ 含「汇报」→ 判定为工作汇报 → 去「生成前确认」问 1/2/3/5。
...
确认问题是硬性必问——Agent 先判断场景，但判断后也必须把对应分支的问题逐一询问用户确认，禁止因已判断而跳过询问或直接生成。确认问题按场景分支：工作汇报 → 问 1/2/3/5；对外展示 → 问 1/2/3/4。如果你发现自己正在生成内容但还没问完这些必须确认的问题，立即停下来，回到确认步骤。
...
3. **输出格式**：逐字说：「要**网页文件**还是**PPT 文件**？」选「PPT 文件」→ 检查本次对话中用户是否已加载**能直接产出 .pptx 版式**的专业PPT制作技能或设计类技能、或调用过**设计类 sub-agent**（客观可查的事实，不是你的能力自评）。**263group-brand-guidelines 本身不算**——它只提供品牌数据，产不出 .pptx 版式：
   - **已加载** → 有原生 PPTX 排版能力，把品牌数据（尺寸/字号/行距/图片规则）作为契约交给该技能/agent，直接按该路径生成；
   - **未加载** → 无原生能力，唯一 PPT 手段是写代码（python-pptx 等）→ 走 3a 把路径选择摆给用户。
   选「网页文件」→ 直接第 5 题（工作汇报）或第 4 题（对外展示）。对外展示分支第 4 题询问视觉风格。
3a. **PPT 输出路径·推荐**（第 3 题选「PPT 文件」且对话中未加载 PPT/设计技能或 agent、唯一手段是代码生成时）：逐字说：「当前工具制作 PPT 文件需要**用Python代码生成**，效果可能不够稳定。推荐改生成**网页文件**，或者使用**专业PPT制作技能或设计类技能**。你希望怎么处理？」用户改选网页文件 → 最终格式 = 网页文件；用户选专业能力 → 按该能力路径生成；用户坚持代码生成 → 走 3b。
3b. **PPT 输出路径·坚持警告**（用户坚持代码生成 PPT 时，生成前）：逐字说：「好的，按你的选择用代码生成 PPT 文件。提醒一下：排版效果可能与**HTML版**有差距，个别页面可能需要你**手动调整**。确认后我开始生成。」→ 此路径下读取 `pptx-python-guide.md` 并按其中要点实现。
4. **视觉风格**（仅对外展示时询问；工作汇报不询问，默认严格按模板排版）：逐字说：「**严格按模板排版**，还是**搭配额外专业美化技能**？推荐**搭配额外专业美化技能**。」选「严格按模板」→ 排版完全按固定模板，不调用外部技能；选「搭配额外专业美化技能」→ 逐字回复「好的，为你搜索专业设计技能并使用」，按第 3 题输出格式定向搜索（选网页文件 → 搜网页设计技能；选 PPT 文件 → 搜专业PPT制作技能；搜索定向见「规则 → 视觉风格 · 搜索定向」）。
5. **封面风格**（仅工作汇报时追加）：逐字说：「封面风格：选**个性化风格**（Geek 像素动效）还是**严谨商务风格**？」推荐逻辑由第 3 题输出格式决定，双向强制：
   - 第 3 题选「PPT 文件」→ 必须追加一句：「如生成 PPT 文件，推荐**严谨商务风格**」（原因：个性化风格的像素动效在 PPT 中无法原生渲染，严谨风格是 PPT 的适配封面）
   - 第 3 题选「网页文件」→ 不追加推荐语，两个风格都支持，由用户自选

> 对外展示命中广告法违禁词时的**打断话术**见「工作流 → 广告法合规审查（仅对外展示）」，命中打断时逐字使用（同样属于唯一输出源）。
> Q4 选「搭配额外专业美化技能」后的**承诺话术**「好的，为你搜索专业设计技能并使用」与**搜索失败回退话术**「当前未找到可用的专业美化技能，将按严格模板排版。」同样逐字使用（唯一输出源，见「规则 → 视觉风格 · 搜索定向」）。
...
| 视觉风格美化、设计 skill 介入、专业设计能力美化排版 | 搭配额外专业美化技能（Q4 话术） |
| 搜索设计技能、加载设计类技能/agent | 好的，为你搜索专业设计技能并使用（Q4 承诺话术）；未找到时用「当前未找到可用的专业美化技能，将按严格模板排版。」（回退话术） |
...
> 允许对用户说的正常词：集团 Logo、网页文件、PPT 文件、集团红、通信蓝、PPTX、正文、标题、副标题、页面、图表、广告法合规审查、违禁词、极限词、专业美化技能、专业设计技能等业务/格式/合规词汇。

### 规则

- 全部必答问题确认前，禁止进入后续任何步骤（大纲收集、封面信息提取、数据图表收集、pages.json 生成、透明声明）
- 禁止假设默认值跳过提问。**Agent 的判断结果不能替代用户确认**——每个分支问题都必须向用户询问，用户未明确回答的选项必须追问
- **PPT 能力检查（第 3 题选「PPT 文件」时）**：原生 PPTX 排版能力只看一条客观事实——本次对话中用户是否已加载**能直接产出 .pptx 版式**的专业PPT制作技能或设计类技能、或调用过设计类 sub-agent。**263group-brand-guidelines 不算**（品牌数据层，只产品牌数据与 HTML）。**你的代码能力（python-pptx 等）也不构成原生能力**。禁止以「我能用 python-pptx 精确实现规范」或「263group-brand-guidelines 就是 PPT 技能」为由跳过 3a/3b；未加载 → 按无原生能力处理，走 3a。
- **视觉风格推荐（第 4 题，仅对外展示）**：对外展示**一律推荐**「搭配额外专业美化技能」（受众在组织外部，专业美化是合理默认）——推荐依据第 3 题输出格式（选 PPT 文件 → 按 PPT 搜索方向；选网页文件 → 按网页搜索方向）。工作汇报不询问第 4 题（默认严格按模板排版），其封面风格推荐维持现状（第 5 题仅 PPT 文件时推荐严谨商务风格，见第 5 题）。
- **视觉风格 · 搜索定向（第 4 题选「搭配额外专业美化技能」后）**：先承诺（「好的，为你搜索专业设计技能并使用」），**搜索按第 3 题输出格式定向执行**（第 3 题已先确认）——选「网页文件」→ 搜索网页/HTML 设计技能（如 frontend-design）；选「PPT 文件」→ 搜索专业PPT制作技能或设计类技能（若对话中已加载该能力，直接按「PPT 能力检查」路径处理，无需搜索）。**搜索载体写死**：对话中已加载的设计类技能/agent → 直接用，不搜索；未加载 → 用技能搜索工具（Skill / Agent）搜索并加载。**搜索不到 → 回退话术**：「当前未找到可用的专业美化技能，将按严格模板排版。」（逐字，唯一输出源），按严格模板路径生成，禁止悄悄自行硬排或假装有设计能力。
- **封面风格推荐必须与第 3 题输出格式联动**：用户选「PPT 文件」才推荐严谨商务风格；用户选「网页文件」禁止推荐（违反 = 交互逻辑错误）。Agent 必须先问完第 3 题拿到输出格式答案，才能决定第 5 题是否补推荐语
- 确认结果必须写入 pages.json：问题 1「工作汇报」→ `"scene": "template"`；「对外展示」→ 省略 `scene`。问题 2「集团红」→ `"colorScheme": "group-red"`；「通信蓝」**暂不可选**（选定即视为集团红，映射 `"colorScheme": "group-red"`——business-blue 未启用，禁止写入）。问题 5「个性化」→ 省略封面 `background`（ASCII 默认）；「严谨」→ 封面 `"background": "red-template"`
...
2. **生成前确认** → 按场景分支逐个询问（见上方「生成前确认」：工作汇报 → 问 1/2/3/5；对外展示 → 问 1/2/3/4）。Agent 先判断场景，但判断后也必须逐一询问用户确认。全部确认后才能继续。禁止跳过，禁止假设默认值
...
### PPTX 输出路径（第 3 题选「PPT 文件」后的决策）

PPTX 布局比 HTML 脆，不鼓励手写代码排版。按优先级决策（交互见「生成前确认 → 用户话术」3a/3b）：
...
2. **未加载 → 推荐生成网页文件（HTML）**：交互见 3a，用户接受则按网页文件生成（可靠）。
3. **用户坚持代码生成 PPT** → 生成前逐字提醒（3b）效果可能有差距；**当且仅当此路径下读取 `pptx-python-guide.md`**（python-pptx 实现要点，把行距/中文字体/图片等比/透明文字框等机械点写死），其他路径不读该文件。**此路径产出即校验——生成脚本末尾必须嵌入 brand-check 交付断言（`pptx-python-guide.md` §9.1），校验不通过不产出，无法事后跳过**。
...
零外部依赖，单一 HTML 文件，浏览器直接打开。播放壳：全屏（F11）/键盘翻页（方向键 ←/→ 前后翻页，Home/End 首尾）。**点击页面任意位置不翻页**——翻页只能靠键盘（2026-08-21 起，汇报场景防误触）。
...
从 `brand-tokens.json` → `chartPalette` 读取，按当前 colorScheme 选择子块（集团红 → `group-red`，通信蓝 → `business-blue`，后者**暂不可选**，当前禁止启用）。
