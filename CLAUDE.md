...
| `brand-check-pptx.py` | PPTX 品牌合规检查（可选工具，python-pptx）：颜色白名单/红底文字=白/结尾页最后/图片内嵌/对外禁微软雅黑 |
...
- **硬门禁**：生成前必须逐项确认，按场景分支（工作汇报 → 场景/配色/输出格式/封面风格；对外展示 → 场景/配色/输出格式/视觉风格——**先问输出格式、再问视觉风格**）。Agent 先判断场景但判断后也必须逐一询问用户确认，禁止跳过、禁止假设默认值。用户话术用固定平实词，禁止暴露内部文件名。**封面风格推荐与输出格式联动**：选 PPT 文件才推荐严谨商务风格；选网页文件禁止推荐。**视觉风格推荐（第 4 题，仅对外展示）**：一律推荐「搭配额外专业美化技能」（受众在外部）；选后逐字承诺「好的，为你搜索专业设计技能并使用」，**搜索按第 3 题输出格式定向**（网页→网页设计技能/PPT→专业PPT制作技能），已加载直接用于不搜，搜不到用回退话术「当前未找到可用的专业美化技能，将按严格模板排版。」，禁止悄悄自排。**PPT 输出路径**：第 3 题选 PPT 文件后——对话中已加载能产出 .pptx 版式的 PPT/设计技能或 agent（263group-brand-guidelines 本身不算）直接用；未加载则走话术 3a 推荐改网页文件或用专业PPT制作技能或设计类技能；用户坚持代码生成则话术 3b 生成前警告效果可能需人工调整，并按 `pptx-python-guide.md` 实现。
...
- **品牌合规审查（生成后闸门）**：**HTML 侧** generate.js 强制校验——**结尾页必须最后**（type==='end'，否则 exit 1）、**custom.html 颜色白名单**（只用 colorSchemes/semantic/chartPalette 色值，非白名单 exit 1）、**对外展示 custom.html 禁微软雅黑**（开源栈，闭源禁止）。**PPTX 侧** SKILL.md「PPTX 生成后自查（硬门禁）」清单逐条核对（颜色白名单读 token 禁手写 hex/红底文字=白/结尾页最后/Logo 内嵌等比/字体分流/封面文字框透明），分两条路径：**4b 代码生成路径** `brand-check-pptx.py` 是生成脚本的**交付断言**（嵌进生成动作，校验不通过 `sys.exit(1)` 不产出，无法事后跳过，见 pptx-guide §9.1）；**设计技能产出路径**只能作可选检具 + 清单人工核对（产出方式在设计技能手中，**无机器强制**——诚实边界，见 pptx-guide §9.2）。无 python-pptx 禁止降级按清单人工自查。品牌色卡/结尾页/Logo/公司数据是底线，**内/外皆强制**；行距/字号/封面对外可放。
...
- **Skill zip**：`测试记录/263-vi-skill-MMDD.zip`，22 文件（含 `cover-red-template.png`、`cover-themed-fallback.png`、`template-cover-bg.png`、`pptx-python-guide.md`、`ad-compliance.json`、`brand-check-pptx.py`）。`brand-tokens.json` v2.5（含 chartPalette 图表色板——唯一 7 档红尺度 + 语义色红涨绿跌 + 两灰唯一命名 + slogan 原生尺寸/宽高比 + 开源字体栈 + themedFallbackCover）。
