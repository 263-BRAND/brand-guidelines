...
- **硬门禁**：生成前必须逐项确认，按场景分支（工作汇报 → 场景/配色/输出格式/封面风格；对外展示 → 场景/配色/视觉风格/输出格式）。Agent 先判断场景但判断后也必须逐一询问用户确认，禁止跳过、禁止假设默认值。用户话术用固定平实词，禁止暴露内部文件名。**封面风格推荐与输出格式联动**：选 PPT 文件才推荐严谨商务风格；选网页文件禁止推荐。**视觉风格推荐（Q3，仅对外展示）**：一律推荐「搭配额外专业美化技能」（受众在外部），推荐不依赖输出格式；选后逐字承诺「好的，为你搜索专业设计技能并使用」，**搜索在 Q4 输出格式确认后按格式定向**（网页→网页设计技能/PPT→专业PPT制作技能），已加载直接用于不搜，搜不到用回退话术「当前未找到可用的专业美化技能，将按严格模板排版。」，禁止悄悄自排。**PPT 输出路径**：Q4 选 PPT 文件后——对话中已加载能产出 .pptx 版式的 PPT/设计技能或 agent（263group-brand-guidelines 本身不算）直接用；未加载则走话术 4a 推荐改网页文件或用专业PPT制作技能或设计类技能；用户坚持代码生成则话术 4b 生成前警告效果可能需人工调整，并按 `pptx-python-guide.md` 实现。
