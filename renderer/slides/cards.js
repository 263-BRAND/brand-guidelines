...
function renderSlide(slide, tokens, pages, index, resolvedBg) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    cardsHtml += '<h3 style="font-size:' + tokens.typography.html.scale.subtitle.template + 'pt;color:' + c.dark + ';font-weight:bold;margin-bottom:8px;">' + esc(item.title) + '</h3>\n';
    cardsHtml += '<p style="font-size:' + tokens.typography.html.scale.body.template + 'pt;color:' + c.gray + ';line-height:1.7;">' + esc(item.description) + '</p>\n';
    cardsHtml += '</div>\n';
  }
