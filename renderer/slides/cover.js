...
function renderTemplate(slide, tokens, pages, index, c) {
  // Convert 1920px-reference pt to vw for responsive rendering
  ...
  var asciiVW = 0.57; // 11pt at 1920px

  var html = '<div class="slide-page" id="s' + index + '" style="background:' + c.lightGray + '; overflow:hidden;">\n';

  // ASCII logo — each line as a span for per-line entrance animation
  ...
  for (var li = 0; li < ASCII_LOGO.length; li++) {
    var dir = (li % 2 === 0) ? 'left' : 'right';
    html += '<span class="ascii-line" style="display:block;opacity:0;transform:translateX(' + (dir === 'left' ? '-' : '') + '60px);transition:opacity 0.6s ease,transform 0.6s ease;">' + esc(ASCII_LOGO[li]) + '</span>\n';
  }
  html += '</pre>\n</div>\n';

  // Title + meta — cover-content for delayed reveal
