...
function renderSlide(slide, tokens, pages, index, resolvedBg) {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    eventsHtml += '<h3 style="font-size:' + tokens.typography.html.scale.subtitle.template + 'pt;color:' + c.dark + ';font-weight:bold;margin-bottom:4px;">' + esc(evt.title) + '</h3>\n';
    if (evt.description) { eventsHtml += '<p style="font-size:' + tokens.typography.html.scale.body.template + 'pt;color:' + c.gray + ';line-height:1.6;">' + esc(evt.description) + '</p>\n'; }
    eventsHtml += '</div>\n</div>\n';
  }
  ...
  html += '<h2 style="font-size:' + tokens.typography.html.scale.contentTitle.template + 'pt;font-weight:bold;color:' + c.dark + ';">' + esc(slide.title) + '</h2>\n';
  html += '</div>\n';
  ...
  html += '<div style="position:absolute;bottom:3%;right:6%;font-size:' + tokens.typography.html.scale.caption.template + 'pt;color:' + c.gray + ';">' + (index + 1) + ' / ' + pages.slides.length + '</div>\n';
