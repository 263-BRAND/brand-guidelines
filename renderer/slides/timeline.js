...
function renderSlide(slide, tokens, pages, index, resolvedBg) {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    eventsHtml += '<span style="font-size:' + tokens.typography.scale.subtitle.template + ';font-weight:bold;color:' + c.primary + ';">' + esc(evt.year) + '</span>\n';
    eventsHtml += '</div>\n';
    ...
    eventsHtml += '<h3 style="font-size:' + tokens.typography.scale.subtitle.template + ';color:' + c.dark + ';font-weight:bold;margin-bottom:4px;">' + esc(evt.title) + '</h3>\n';
    if (evt.description) { eventsHtml += '<p style="font-size:' + tokens.typography.scale.body.template + ';color:' + c.gray + ';line-height:1.6;">' + esc(evt.description) + '</p>\n'; }
    eventsHtml += '</div>\n</div>\n';
  }
  ...
  if (slide.sectionLabel) {
    html += '<div style="font-size:' + tokens.typography.scale.caption.template + ';color:' + c.primary + ';letter-spacing:2px;margin-bottom:4px;">' + esc(slide.sectionLabel) + '</div>\n';
  }
  html += '<h2 style="font-size:' + tokens.typography.scale.contentTitle.template + ';font-weight:bold;color:' + c.dark + ';">' + esc(slide.title) + '</h2>\n';
  html += '</div>\n';
  ...
  html += '<div style="position:absolute;bottom:3%;right:6%;font-size:' + tokens.typography.scale.caption.template + ';color:' + c.gray + ';">' + (index + 1) + ' / ' + pages.slides.length + '</div>\n';
