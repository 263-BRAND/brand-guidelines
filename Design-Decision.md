...
2. **硬门禁确认** → 按场景分支逐一询问（内部 → 场景/配色/输出格式/封面风格；对外 → 场景/配色/视觉风格/输出格式）。**Agent 先判断场景，但判断后也必须询问用户确认，禁止跳过**。用户话术固定平实词，禁止暴露内部文件名（见 SKILL.md「生成前确认」）。**交互措辞三铁律**：① 确认问题逐字原样用固定话术；② 禁止向用户播报判断依据（命中/锚词/锁定等推理词不出现）；③ 内部词汇与文件名的「用户话术替换对照」见 SKILL.md「用户对话措辞规范 → 对话黑名单」
